...
This guide contains everything you need to get started with enabling logging for your existing Gemini API applications. In this guide you'll learn how to view logs from an existing or new application in the Google AI Studio dashboard to better understand model behavior and how users may be interacting with your applications. Use logging to observe, debug, and *optionally share usage feedback with Google to help improve Gemini across developer use cases*.[\*](/gemini-api/docs/logs-policy)

All `GenerateContent` and `StreamGenerateContent` API calls are supported, including those made through [OpenAI compatibility](/gemini-api/docs/openai) endpoints.
...
![](/static/gemini-api/docs/images/logs-state.png)
