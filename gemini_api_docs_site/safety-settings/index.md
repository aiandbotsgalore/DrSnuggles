...
These categories are defined in [`HarmCategory`](/api/rest/v1/HarmCategory). The Gemini models only support `HARM_CATEGORY_HARASSMENT`, `HARM_CATEGORY_HATE_SPEECH`, `HARM_CATEGORY_SEXUALLY_EXPLICIT`, and `HARM_CATEGORY_DANGEROUS_CONTENT`. All other categories are used only by PaLM 2 (Legacy) models.
