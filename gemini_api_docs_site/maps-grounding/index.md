...
Grounding with Google Maps is exposed through the Gemini API as a tool within the [`generateContent`](/api/generate-content) method. You enable and configure Grounding with Google Maps by including a [`googleMaps`](/api/caching#GoogleMaps) object in the `tools` parameter of your request.
...
The [`googleMaps`](/api/caching#GoogleMaps) tool can additionally accept a boolean `enableWidget` parameter, that is used to control whether the [`googleMapsWidgetContextToken`](/api/generate-content#GroundingMetadata) field is returned in the response. This can be used to display a [contextual Places widget](https://developers.google.com/maps/documentation/javascript/reference/places-widget).
...
The Gemini API returns the following information with the [`groundingMetadata`](/api/generate-content#GroundingMetadata):
...
![An example of a maps widget when rendered](/static/gemini-api/docs/images/maps/maps-widget.png)
...
*   Attribute each source to Google Maps following the Google Maps text [attribution guidelines](#maps-attribution-guidelines).
...
*   To learn about sp, see Font size units on the [Material Design website](https://m3.material.io/styles/typography/type-scale-tokens#3f4488e7-3b74-45b0-a143-9d6afa4d62dc).
...
You can find their capabilities on the [model overview](/gemini-api/docs/models) page.
...
| [Gemini 2.0 Flash](/gemini-api/docs/models#gemini-2.0-flash) | ✔️ |
...
*   To learn more about responsible AI best practices and Gemini API's safety filters, see [the Safety settings guide](/gemini-api/docs/safety-settings).
