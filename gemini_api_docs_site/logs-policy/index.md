...
This page outlines the storage and management of [Gemini API logs](/gemini-api/docs/logs-datasets), which are developer-owned API data from supported Gemini API calls for projects with billing enabled. Logs encompass the entire process from a user's request to the model's response.
