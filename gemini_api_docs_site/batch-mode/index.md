...
The following example calls the [`BatchGenerateContent`](/api/batch-mode#google.ai.generativelanguage.v1beta.BatchService.BatchGenerateContent) method with the input file uploaded using File API:
...
If you're using [Gemini Nano Banana](/gemini-api/docs/image-generation) and need to generate a lot of images, you can use the Batch API to get higher [rate limits](/gemini-api/docs/rate-limits) in exchange for a turnaround of up to 24 hours.
...
*   **Error handling:** Check the `batchStats` for `failedRequestCount` after a job completes. If using file output, parse each line to check if it's a `GenerateContentResponse` or a status object indicating an error for that specific request. See the [troubleshooting guide](/gemini-api/docs/troubleshooting#error-codes) for a complete set of error codes.
