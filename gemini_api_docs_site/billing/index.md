...
Billing for the Gemini API is based on two pricing tiers: *free of charge* (or *free*) and *pay-as-you-go* (or *paid*). Pricing and rate limits differ between these tiers and also vary by model. You can check out the [rate limits](/gemini-api/docs/rate-limits) and [pricing](/gemini-api/docs/pricing) pages for more into. For a model-by-model breakdown of capabilities, see the [Gemini models page](/gemini-api/docs/models/gemini).
